...
function begin(details){
	...
	if (details.totalTests > 0) {
		console.log(`Monio Test Suite (${details.totalTests})`);
		console.log("");
	}
	else {
		console.log(`Monio Test Suite: empty!`);
		process.exit(1);
	}
}
...
function testDone(results){
	...
	if (results.failed > 0) {
		console.log(`Failed: ${results.module} | '${results.name}' (${results.failed}/${results.total})`);
		...
		console.log(`Passed: ${results.module} | '${results.name}' (${results.passed}/${results.total})`);
	}
	else {
		console.log(`No assertions run: ${results.module} | '${results.name}'`);
	}
}
